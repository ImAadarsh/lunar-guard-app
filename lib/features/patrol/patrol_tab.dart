                ...
                : () async {
                    ...
                    if (id == null || id <= 0) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text('Enter a valid checkpoint ID.')),
                      );
                      return;
                    }
                    ...
                    final err = await patrolController.submitCheckpointId(id);
                    ...
                    if (err != null) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text('Checkpoint scan submitted.')),
                      );
                    }
                  },
            style: FilledButton.styleFrom(
                ...
                children: [
                  ...
                  if (patrol.scans.isEmpty)
                    ...
                    ...patrol.scans.take(6).map(
                          (s) => _ScanTile(
                            label:
                                '${s.checkpointLabel ?? 'Checkpoint #${s.checkpointId}'}'
...
class _ScanTile extends StatelessWidget {
  const _ScanTile(
      {required this.label, required this.time, required this.done});
